...
namespace System.Web.Mvc
{
  ...
  public static class HtmlHelperExtensions
  {
    ...
    public static MvcHtmlString ValidationErrorFor<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, string error)
    {
      ...
      return HasError(htmlHelper, ModelMetadata.FromLambdaExpression(expression, htmlHelper.ViewData), modelName)
        ? new MvcHtmlString(error)
        : null;
    }

    public static MvcHtmlString ValidationErrorFor(this HtmlHelper htmlHelper, string modelName, string error)
    {
      return HasError(htmlHelper, ModelMetadata.FromStringExpression(modelName, htmlHelper.ViewData), modelName)
        ? new MvcHtmlString(error)
        : null;
    }

    private static bool HasError(this HtmlHelper htmlHelper, ModelMetadata modelMetadata, string modelName)
    {
