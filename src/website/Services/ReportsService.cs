...
namespace Kcsara.Database.Services
{
  ...
  public class ReportsService : IReportsService
  {
    ...
    /// <returns></returns>
    public Stream GetMissionReadyList(UnitRow unit)
    {
    ...
    public Stream GetMembershipReport()
    {
      ...
      //IQueryable<MemberRow> members = this.db.Members.Include("Addresses", "Memberships", "ComputedAwards.Course").Where(f => f.Memberships.Any(g => g.Status.IsActive && g.EndTime == null));
      ...
      //  Action<MemberRow, string> doContact = (member, type) =>
      //  {
    ...
    private void GenerateMissionReadySheets(UnitRow unit, ExcelFile xl, ExcelSheet goodList)
    {
      throw new NotImplementedException("reimplement");
