...
namespace Kcsar.Database.Data
{
  ...
  public class MemberContactRow : ModelObjectRow
  {
    ...
    public MemberContactRow()
      : base()
    {
      this.Type = MemberContactRow.AllowedTypes[0];
      this.Subtype = MemberContactRow.GetSubTypes(this.Type).SubTypes.FirstOrDefault() ?? "";
    }
    ...
    {
      return string.Format("<b>{0}</b> {1} contact: {2} {3}", this.Member.FullName, this.Type, this.Subtype, this.Value);
    }
    ...
    public override IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
      ...
      else if (!MemberContactRow.AllowedTypes.Contains(this.Type.ToLower()))
      {
        yield return new ValidationResult("Must be one of: " + string.Join(", ", MemberContactRow.AllowedTypes), new[] { "Type" });
      }

      ContactInfoSubType subtypes = MemberContactRow.GetSubTypes(this.Type);
      ...
      else if (!MemberContactRow.TryParse(this.Type, this.Subtype, this.Value, true, out dummy))
      {
