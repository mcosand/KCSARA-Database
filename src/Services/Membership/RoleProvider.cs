...
namespace Kcsar.Membership
{
  ...
  public class RoleProvider : SqlRoleProvider, INestedRoleProvider
  {
    ...
    public string[] GetRolesWithRole(string child)
    {
      ...
      using (SqlConnection connection = new SqlConnection(this.connectionString))
      {
        ...
        SqlCommand cmd = new SqlCommand("SELECT p.rolename FROM RolesInRoles as j JOIN aspnet_Roles as p ON j.ParentRoleId=p.RoleId JOIN aspnet_Roles as c ON j.ChildRoleId=c.RoleId WHERE c.RoleName='" + child + "'", connection);
        connection.Open();
