...
namespace Kcsara.Database.Web.Controllers
{
    ...
    public partial class MissionsController
    {
        ...
        {
            return View(context.Missions.First(f => f.Id == id));
        }
        ...
        {
            Mission mission = context.Missions.Single(f => f.Id == id);
            string kind = null;
            string desc = null;
                    ...
                    else
                    {
                        foreach (var track in tracks.Cast<XmlNode>())
                        {
                            ...
                            if (points != null && points.Count > 2)
                            {
                                ...
                                context.MissionGeography.Add(geog);
                            }
                        }
                    }
                }
        ...
        public ActionResult DeleteWaypoint(Guid id)
        {
            ...
            {
                var waypoint = (from w in ctx.MissionGeography where w.Id == id select w).First();
                ctx.MissionGeography.Remove(waypoint);
                ctx.SaveChanges();
            }
        ...
        public ActionResult SubmitWaypoint(WaypointView wpt)
        {
            ...
            using (var ctx = GetContext())
            {
                geog = (from g in ctx.MissionGeography where g.Id == wpt.Id select g).FirstOrDefault();
                if (geog == null)
                {
                    geog = new MissionGeography { Mission = (from m in ctx.Missions where m.Id == wpt.EventId select m).First() };
                    ctx.MissionGeography.Add(geog);
                }
        ...
        public JsonDataContractResult GetGeographies(DateTime? start, DateTime? stop, bool? overview)
        {
            ...
            {
                var query = (from g in c.MissionGeography.Include("Mission") select g);
                if (start.HasValue)
                {
        ...
        public DataActionResult GetGeography(Guid id, bool? overview)
        {
            ...
            using (var c = GetContext())
            {
                var query = (from g in c.MissionGeography where g.Mission.Id == id select g);
        ...
        public FileContentResult MissionsKML(DateTime? begin, DateTime? end)
        {
            ...
            Dictionary<Guid, Tuple<int, int>> detailCounts;
            using (var ctx = GetContext())
            {
                detailCounts = (from mg in ctx.MissionGeography group mg by mg.Mission.Id into g select new { Id = g.Key, Points = g.Count(f => !f.Kind.EndsWith("trk")), Tracks = g.Count(f => f.Kind.EndsWith("trk")) }).ToDictionary(f => f.Id, f => new Tuple<int, int>(f.Points, f.Tracks));
            }
        ...
        public FileContentResult MissionKML(Guid id)
        {
            ...
            using (var ctx = GetContext())
            {
                m = (from mission in ctx.Missions where mission.Id == id select mission).FirstOrDefault();
            }
        ...
        public ActionResult GetGeographyInstances(Guid missionId, string kind)
        {
            ...
            using (var ctx = GetContext())
            {
                ...
                {
                    result = (from g in ctx.SubjectGroups where g.Mission.Id == missionId select g).ToDictionary(f => f.Id, f => "Group " + (f.Number - 1));
                }
            }
