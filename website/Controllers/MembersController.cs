...
//        [FilterTypes(FilterTypes.ActiveOnly | FilterTypes.Time | FilterTypes.Unit)]
        public ActionResult Index()
        {
            ...
            using (var ctx = GetContext())
            {
                ...
                var members = ctx.GetActiveMembers(null, now, "Memberships.Unit", "Memberships.Status");
                var model = members.ToArray().Select(f => new MemberSummaryRow(f)
                {
                    Units = f.Memberships.Where(ctx.GetActiveMembershipFilter(null, now)).ToDictionary(g => g.Unit.Id, g => g.Unit.DisplayName)
                });
        ...
        public ActionResult Detail(Guid id)
        {
            ...
            Member member = (from m in context.Members.Include("Memberships.Unit").Include("Memberships.Status")
        ...
        {
            Member member = GetMember(context.Members.Include("TrainingAwards").Include("TrainingAwards.Course"), id);
        ...
        public ActionResult NewEsarTrainee(FormCollection fields)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    context.SaveChanges();
                    context.RecalculateTrainingAwards(m.Id);
                    context.SaveChanges();
        ...
        private Member NewEsarTrainee_Internal(FormCollection fields)
        {
            ...
            SarUnit esar = (from u in context.Units where u.DisplayName == "ESAR" select u).First();
            UnitStatus status = (from s in context.UnitStatusTypes where s.Unit.Id == esar.Id && s.StatusName == "trainee" select s).First();
            ...
            if (!string.IsNullOrEmpty(fields["Street"]))
            {
                ...
                context.PersonAddress.Add(address);
            }
            ...
            context.UnitMemberships.Add(um);

            TrainingCourse courseA = (from tc in context.TrainingCourses where tc.DisplayName == "Course A" select tc).First();
            DateTime nextDate = courseDate.AddDays(1);

            Training t = (from trn in context.Trainings where trn.StartTime >= courseDate && trn.StartTime < nextDate && trn.Title == "Course A" select trn).FirstOrDefault();
            if (t == null)
            {
                t = new Training();
            ...
            context.TrainingRosters.Add(tr);
            t.Roster.Add(tr);
        ...
        public ActionResult CardDatabaseDifferences(bool? commit, Guid[] except)
        {
            ...
            dbMembers = context.Members.Where(f => f.InternalWacLevel > 0 && f.Memberships.Any(g => g.EndTime == null && g.Status.IsActive) && f.ExternalKey1 == null)
                .OrderBy(f => f.LastName).ThenBy(f => f.FirstName).ToList();
            ...
            {
                var matches = context.Members.Where(f => f.ExternalKey1 == cardMember.PK);

                if (matches.Count() < 1)
                {
                    matches = context.Members.Where(f => f.LastName == cardMember.LastName && f.FirstName == cardMember.FirstName);
                }

                if (matches.Count() > 1)
                {
                    matches = context.Members.Where(f => f.DEM == cardMember.IdentityCode && f.LastName == cardMember.LastName && f.FirstName == cardMember.FirstName);
                }
                else if (matches.Count() == 0)
                {
                    matches = context.Members.Where(f => f.DEM == cardMember.IdentityCode && f.LastName == cardMember.LastName);
                }
            ...
            if (commit ?? false) context.SaveChanges();
        ...
        public ActionResult Badges(string id)
        {
            ...
            var x = (from m in context.Members/*.Include("Memberships.Unit").Include("ComputedAwards").Include("ComputedAwards.Course").Include("Memberships.Status")*/ select m).WhereIn(c => c.Id, guids).OrderBy(o => o.LastName + "," + o.FirstName);
        ...
        public ActionResult PhotoUpload(string id)
        {
            ...
            var x = context.Members.Where(GetSelectorPredicate<Member>(ids)).OrderBy(f => f.LastName + "," + f.FirstName);
        ...
        public ActionResult PhotoPreview()
        {
            ...
            var m = context.Members.Where(GetSelectorPredicate<Member>(images.Keys)).OrderBy(f => f.LastName + "," + f.FirstName);
        ...
        public ActionResult PhotoCommit(FormCollection fields)
        {
            ...
            else
            {
                ...
                var members = context.Members.Where(GetSelectorPredicate<Member>(images.Keys)).OrderBy(f => f.LastName + "," + f.FirstName);
        ...
        public ActionResult SuggestDem()
        {
            ...
            var x = (from m in context.Members orderby m.DEM.Length, m.DEM select m.DEM);
        ...
        public ActionResult GetMembersActiveUnits(Guid id)
        {
            ...
            Member m = GetMember(context.Members.Include("Memberships").Include("Memberships.Status").Include("Memberships.Unit"), id);
        ...
        public ActionResult GetMembersAnimals(Guid id)
        {
            ...
            AnimalView[] animals = (from ao in context.AnimalOwners.Include("Animal").Include("Owner") where ao.Owner.Id == id && (ao.Ending == null || ao.Ending > atTime) select new AnimalView
            {
             Id = ao.Animal.Id,
             Type = ao.Animal.Type,
             Name = ao.Animal.Name,
        ...
        public ActionResult CardData(FormCollection columns)
        {
            ...
            var members = (from m in context.Members.Include("Memberships.Unit").Include("Memberships.Status")
            ...
            var links = (from ol in context.AnimalOwners.Include("Animal").Include("Owner").Include("Owner.Memberships.Unit").Include("Owner.Memberships.Status") where ol.IsPrimary && ol.Ending == null orderby ol.Animal.Id select ol);
        ...
        {
            return GetMember(context.Members, id);
        }
        ...
        private ActionResult InternalSaveMembership(UnitMembership um, FormCollection fields)
        {
            try
            {
                ...
                UnitStatus status = (from s in context.UnitStatusTypes where s.Id == statusId select s).First();
                um.Status = status;
        ...
        private void UpdateMemberships(Guid personId)
        {
            ...
            foreach (Kcsar.Database.Model.UnitMembership um in (from u in context.UnitMemberships.Include("Person").Include("Unit") where u.Person.Id == personId select u).OrderBy(f => f.Unit.Id).ThenBy(f => f.Activated))
            {
                if (lastUm != null)
                {
        ...
        public ActionResult DeleteMembership(Guid id, FormCollection fields)
        {
            ...
            context.UnitMemberships.Remove(um);
            context.SaveChanges();
        ...
        {
            return GetUnitMembership(context.UnitMemberships.Include("Person").Include("Unit").Include("Status"), id);
        }
        ...
        public ActionResult CreateAddress(Guid personId)
        {
            ...
            address.Person = (from p in context.Members where p.Id == personId select p).First();
        ...
        {
            PersonAddress address = (from a in context.PersonAddress.Include("Person") where a.Id == id select a).First();
        ...
        public ActionResult DeleteAddress(Guid id, FormCollection fields)
        {
            ...
            context.PersonAddress.Remove(address);
            context.SaveChanges();
        ...
        public DataActionResult GetContacts(Guid id)
        {
            ...
            {
                model = (from c in ctx.PersonContact
        ...
        public DataActionResult PromoteContact(Guid id)
        {
            ...
            using (var ctx = GetContext())
            {
                var promotee = (from c in ctx.PersonContact.Include("Person") where c.Id == id select c).FirstOrDefault();
                ...
                promotee.Priority = 0;
                foreach (var contact in (from c in ctx.PersonContact where c.Person.Id == promotee.Person.Id && c.Type == promotee.Type && c.Priority == 0 select c))
                {
                    contact.Priority = 1;
                }
                try
        ...
        public DataActionResult SubmitContact(/*[ModelBinder(typeof(JsonDataContractBinder<MemberContactView>))] */MemberContactView view)
        {
            ...
            using (var ctx = GetContext())
            {
                model = (from c in ctx.PersonContact.Include("Person") where c.Id == view.Id select c).FirstOrDefault();
                if (model == null)
                {
                    model = new PersonContact();
                    model.Priority = 0;
                    if ((from c in ctx.PersonContact where c.Person.Id == view.MemberId && c.Type == view.Type select c.Id).Count() > 0)
                    {
                        model.Priority = 1;
                    }
                    ctx.PersonContact.Add(model);
                }
                ...
                try
                {
                    ...
                    if (model.Person == null || model.Person.Id != view.MemberId) model.Person = (from m in ctx.Members where m.Id == view.MemberId select m).FirstOrDefault();
        ...
        public DataActionResult DeleteContact(Guid id)
        {
            ...
            using (var ctx = GetContext())
            {
                var match = (from c in ctx.PersonContact where c.Id == id select new { Id = c.Person.Id, Type = c.Type }).First();

                var contacts = (from c in ctx.PersonContact.Include("Person") where c.Person.Id == match.Id && c.Type == match.Type select c).ToList();
                ...
                ctx.PersonContact.Remove(contact);
                ctx.SaveChanges();
            }
        ...
        public ActionResult GetGeographies(Guid? unitId)
        {
            ...
            using (var c = GetContext())
            {
                ...
                var geographies = context.Members
                    .Where(f => f.Memberships.Any(g => (g.Unit.Id == unitId || unitId == null) && g.Status.IsActive && g.EndTime == null))
                ...
                foreach (var row in geographies)
                {
                    ...
                    if (row.Geo == lastGeo)
                    {
                        view.Description = PersonDescription(row.Person, c) + view.Description;
                        continue;
                    }
                    lastGeo = row.Geo;
                    ...
                    view.Description = PersonDescription(row.Person, c) + string.Join("<br/>", parts.Skip(1));
        ...
        private string PersonDescription(Member m, KcsarContext ctx)
        {
        ...
        public ActionResult PromotionsResult(List<Guid> m, bool? relative)
        {
            ...
            using (var ctx = GetContext())
            {
                ...
                names = ctx.Members.WhereIn(f => f.Id, m).OrderBy(f => f.LastName).ThenBy(f => f.FirstName).ThenBy(f => f.Id).Select(f => f.FirstName + " " + f.LastName).ToList();

                var courses = (from c in ctx.TrainingCourses where c.Unit.Id == esar && c.Categories.Contains("leadership") select c);

                var responses = ctx.MissionRosters.WhereIn(f => f.Person.Id, m).Where(f => f.InternalRole == "field" || f.InternalRole == "ol").Where(f => f.TimeIn != null && f.TimeOut != null).OrderBy(f => f.Person.LastName).ThenBy(f => f.Person.FirstName).ThenBy(f => f.Person.Id).ThenBy(f => f.TimeIn);
                ...
                foreach (var row in responses)
                {
                    ...
                    if (personId != lastId)
                    {
                        ...
                        records = ctx.TrainingAward.Include("Course").WhereIn(f => f.Course.Id, courses.Select(f => f.Id)).Where(f => f.Member.Id == personId).OrderBy(f => f.Completed).ToList();
                        recordRow = 0;
                        hoursSum = 0;
                        lastId = personId;
        ...
        public ActionResult ReconcileMemberStatus(string id)
        {
            ...
            var model = (from m in context.Members.Include("Memberships.Unit").Include("Memberships.Status") where m.Memberships.Any(f => f.Unit.Id == unitId && (!f.EndTime.HasValue || f.EndTime > oneYear)) && m.InternalWacLevel != (int)WacLevel.None select m)
                .OrderBy(f => f.LastName).ThenBy(f => f.FirstName).ToList();
            ...
            foreach (Member m in model)
            {
                ...
                if (m.Memberships.Any(f => f.Unit.Id == unitId && f.Status.IsActive && f.EndTime == null))
                {
                    ...
                    {
                        Member mbr = context.Members.Include("ComputedAwards.Course").First(f => f.Id == m.Id);
                    ...
                    else if (m.WacLevel == WacLevel.Field || m.WacLevel == WacLevel.Support)
                    {
                        ...
                        int missions = context.MissionRosters.Where(f => f.Person.Id == m.Id && f.TimeIn > missionCutoff && f.InternalRole != "Responder").Count();
        ...
        public ActionResult ReconcileEmergencyWorkers(bool? saveFile)
        {
            ...
            using (var ctx = GetContext())
            {
                var members = ctx.Members.Include("Memberships.Status").Include("Memberships.Unit").Include("ComputedAwards.Course");
                                ...
                                new ReconcileEmergencyWorkersRow
                                    {
                                        ...
                                        MissionCount = ctx.MissionRosters.Where(g => g.Person.Id == member.Id && g.Mission.StartTime > eighteenMonths).Select(g => g.Mission.Id).Distinct().Count(),
                                        TrainingCount = ctx.TrainingRosters.Where(g => g.Person.Id == member.Id && g.Training.StartTime > eighteenMonths).Select(g => g.Training.Id).Distinct().Count()
                                    }));
                        }
                    }
                    model.RowCount++;
        ...
        public ActionResult PromoteNovices(bool? commit)
        {
            ...
            var promotions = context.Members.Where(f => 
            ...
            if (commit ?? false)
            {
                ...
                context.SaveChanges();
                context.RecalculateTrainingAwards(promotions);
                context.SaveChanges();
