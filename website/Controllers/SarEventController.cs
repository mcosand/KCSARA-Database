...
//        protected abstract ObjectQuery<E> GetSummarySource();
   ...
   //     protected abstract ObjectQuery<R> RostersQuery { get; }
        ...
        public ActionResult EditRoster(Guid id, FormCollection fields)
        {
            ...
            else
            {
                ...
                foreach (string key in fields.AllKeys)
                {
                    ...
                    {
                        Member person = (from mbr in context.Members where mbr.Id == personId select mbr).FirstOrDefault();
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                     ...
                     //   context.Refresh(RefreshMode.StoreWins, this.RostersQuery);
                        context.Dispose();
                        context = GetContext();
        ...
        public ActionResult ICS211(Guid id)
        {
            ...
            using (KcsarContext ctx = GetContext())
            {
                ...
                var source = (model.SarEvent is Mission)
                    ? ctx.MissionRosters.Where(f => f.Mission.Id == id).Select(f => f.Person.Id).Distinct()
                    : ctx.TrainingRosters.Where(f => f.Training.Id == id).Select(f => f.Person.Id).Distinct();
        ...
        public ActionResult Review4x4Roster(bool? bot)
        {
            ...
            {
                SarUnit unit = ctx.Units.Where(f => f.DisplayName == "4x4").Single();
                //ctx.Detach(unit);
        ...
        public ActionResult Submit4x4Roster(Guid missionId)
        {
            ...
            //using (var ctx = GetContext())
            //{
                SarUnit unit = context.Units.Where(f => f.DisplayName == "4x4").Single();
