  ...
  using Kcsara.Database.Web.Model;
    ...
    public partial class MissionsController : SarEventController<Mission, MissionRoster>
    {
        ...
        public ActionResult Subjects(Guid id)
        {
            ...
            return View("Subjects", (from b in context.SubjectGroups.Include(f => f.SubjectLinks.Select(g => g.Subject)).Include(f => f.Mission.SubjectGroups) where b.Mission.Id == id select b));
        }
        ...
        {
            return View("SubjectGroup", (from b in context.SubjectGroups where b.Id == id select b).First());
        }
        ...
        //    link.Group = (from b in context.SubjectGroups where b.Id == id select b).First();
        ...
        private ActionResult _CreateSubject(Guid? groupId, Guid? missionId)
        {
            ...
            if (groupId.HasValue)
            {
                ...
                group = (from b in context.SubjectGroups.Include(f => f.SubjectLinks).Include(f => f.Mission) where b.Id == groupId select b).First();
            }
            else
            {
                ViewData["MissionId"] = missionId.ToString();
                group = new SubjectGroup { Mission = (from m in context.Missions where m.Id == missionId select m).First() };
            }

            subject.FirstName = "Unknown";

            subject.GroupLinks.Add(new SubjectGroupLink { Group = group, Subject = subject });
            //group.Mission = (from p in context.Missions where p.Id == id select p).First();
            //SubjectGroupLink link = new SubjectGroupLink{
        ...
        public ActionResult CreateSubject(Guid? id, FormCollection fields)
        {
            ...
            if (!string.IsNullOrEmpty(fields["GroupId"]))
            {
                ...
                group = (from b in context.SubjectGroups.Include(f => f.Mission).Include(f => f.SubjectLinks) where b.Id == bId select b).First();
                mission = group.Mission;
            }
            else if (!string.IsNullOrEmpty(fields["MissionId"]))
            {
                Guid mId = new Guid(fields["MissionId"]);
                mission = GetEvent(context.Missions.Include(f => f.SubjectGroups), mId);
            ...
            context.SubjectGroupLinks.Add(link);

            //MissionLog log = new MissionLog();
            //log.Mission = (from p in context.Missions where p.Id == missionId select p).First();
            //context.AddToMissionLog(log);
        ...
        {
            // MissionLog log = (from a in context.MissionLog.Include("Mission") where a.Id == id select a).First();
            Subject subject = (from s in context.Subjects where s.Id == id select s).First();
        ...
        public ActionResult Details(Guid id)
        {
            ...
            var details = (from d in context.MissionDetails.Include(f => f.Mission) where d.Mission.Id == id select d).FirstOrDefault();

            if (details == null)
            {
                details = new MissionDetails { Mission = (from m in context.Missions where m.Id == id select m).First() };
            }
        ...
        {
            MissionDetails details = (from md in context.MissionDetails.Include(f => f.Mission) where md.Mission.Id == missionId select md).FirstOrDefault();
            if (details == null)
            {
                details = new MissionDetails { Mission = (from m in context.Missions where m.Id == missionId select m).First() };
                context.MissionDetails.Add(details);
            }
            return details;
        }
        ...
        {
            var group = (from sb in context.SubjectGroups.Include(f => f.Mission).Include(f => f.SubjectLinks.Select(g => g.Subject)) where sb.Id == id select sb).First();
        ...
        private ActionResult InternalSaveSubject(Subject subject, FormCollection fields)
        {
            try
            {
                ...
                //    Mission mission = (from m in context.Missions where m.Id == missionId select m).First();
                //    log.Mission = mission;
        ...
        {
            SubjectGroupLink link = (from l in context.SubjectGroupLinks.Include(f => f.Group.Mission).Include(f => f.Subject) where l.Id == id select l).First();
            return View(link);
        }
        ...
        {
            SubjectGroupLink link = (from l in context.SubjectGroupLinks.Include(f => f.Group.Mission).Include(f => f.Group.SubjectLinks).Include(f => f.Subject.GroupLinks) where l.Id == id select l).First();
            Mission m = link.Group.Mission;

            if (link.Subject.GroupLinks.Count == 1)
            {
                context.Subjects.Remove(link.Subject);
            }

            SubjectGroup group = link.Group;
            if (link.Group.SubjectLinks.Count == 1)
            {
                context.SubjectGroups.Remove(link.Group);
                group = null;
            }

            context.SubjectGroupLinks.Remove(link);
        ...
        {
            SubjectGroupLink link = (from l in context.SubjectGroupLinks.Include(f => f.Group.Mission) where l.Id == id select l).First();

            foreach (var x in (from l in context.SubjectGroupLinks where l.Group.Id == link.Group.Id && l.Number == (link.Number + direction) select l))
            {
                x.Number -= direction;
            }
            ...
            return RedirectToAction("Subjects", new { id = link.Group.Mission.Id });
        }
        ...
        public ActionResult MoveSubjectToGroup(Guid id, int newGroup)
        {
            ...
            link.Group = (from b in context.SubjectGroups where b.Mission.Id == link.Group.Mission.Id && b.Number == newGroup select b).FirstOrDefault();
            if (link.Group == null)
            {
                link.Group = oldGroup.CreateCopy();
                link.Group.Number = newGroup;

                context.SubjectGroups.Add(link.Group);
                RenumberSubjectGroups(m);
            }
            ...
            {
                oldGroup.Mission.SubjectGroups.Remove(oldGroup);
                context.SubjectGroups.Remove(oldGroup);
            }
        ...
        {
            var navProp = context.Entry(group).Reference(f => f.SubjectLinks);
        ...
        {
            //var navProp = context.SubjectGroups.l.Entry(mission).Reference(f => f.SubjectGroups);
            //if (!navProp.IsLoaded) navProp.Load();
        ...
        public JsonDataContractResult SubmitLog(LogSubmission log)
        {
            ...
            try
            {
                ...
                {
                    Mission = context.Missions.Where(f => f.Id == log.MissionId).First(),
                    Data = log.Message,
                    Time = log.Time.Value
                };
                if (log.Person != null)
                {
                    newLog.Person = context.Members.Where(f => f.Id == log.Person.Id).First();
                }
                result = newLog.Id;

                context.MissionLog.Add(newLog);
                context.SaveChanges();
        ...
        {
            MissionLog log = (from a in context.MissionLog.Include(f => f.Person).Include(f => f.Mission) where a.Id == id select a).First();
            return InternalEditLog(log);
        }
        ...
        {
            MissionLog log = (from a in context.MissionLog.Include(f => f.Mission) where a.Id == id select a).First();
            return InternalSaveLog(log, fields);
        }
        ...
        public ActionResult DeleteLog(Guid id)
        {
            ...
            return View((from a in context.MissionLog.Include(f => f.Person) where a.Id == id select a).First());
        }
        ...
        {
            MissionLog log = (from a in context.MissionLog where a.Id == id select a).First();
            context.MissionLog.Remove(log);
            context.SaveChanges();
        ...
        public override ActionResult List(string id)
        {
            ...
            using (var ctx = GetContext())
            {
                ...
                var yearsData = ctx.Missions.Where(f => f.StartTime > eon).Select(f => f.StartTime.Year).Distinct().OrderByDescending(f => f).ToArray();
                ViewData["years"] = yearsData;
                if (yearsData.Length > 0)
                {
                    ViewData["minYear"] = ctx.Missions.Where(f => f.StartTime > eon).Min(f => f.StartTime).Year;
                    ViewData["maxYear"] = ctx.Missions.Max(f => f.StartTime).Year;
                }
                else
                {
                ...
                IQueryable<Mission> source = ctx.Missions;
                if (unit.HasValue)
                {
                ...
                model = (from m in source
                         join mr in ctx.MissionRosters on m.Id equals mr.Mission.Id into Joined
                ...
                ViewData["unit"] = UnitsController.GetUnitSelectList(ctx, unit);
            }
        ...
        public DataActionResult GetRosterHours(Guid id, Guid? unitId)
        {
            ...
            using (var ctx = GetContext())
            {
                IQueryable<MissionRoster> source = ctx.MissionRosters;
                if (unitId.HasValue)
                {
        ...
        //{
        //    return context.Missions;//.Include("Roster.Unit").Include("Roster.Animals").Include("Roster.Person.Animals.Animal");
        //}

        //protected override ObjectQuery<MissionRoster> RostersQuery
        //{
        //    get { return context.MissionRosters; }
        //}

        //protected override void AddEventToContext(Mission newEvent)
        //{
        //    context.AddToMissions(newEvent);
        //}
        ...
        protected override void OnBuildingRosterModel(Guid id)
        {
            ...
            ViewData["missionId"] = id;
            ViewData["UnitList"] = (from u in context.Units orderby u.County + ":" + u.DisplayName select u).ToArray();
        ...
        protected override void OnDeletingRosterRow(MissionRoster row)
        {
            ...
            {
                context.AnimalMissions.Remove(row.Animals.First());
            }
        ...
        protected override void OnProcessingRosterInput(MissionRoster row, FormCollection fields)
        {
            ...
            var unit = (from u in context.Units where u.Id == unitId select u).First();
            ...
            List<AnimalMission> animalsToRemove = new List<AnimalMission>();
            if (string.IsNullOrEmpty(animal))
            {
                foreach (AnimalMission am in row.Animals)
                {
            ...
            else
            {
                ...
                foreach (string newAnimalId in animal.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    Animal a = (from x in context.Animals where x.Id == animalId select x).First();
                    AnimalMission am = new AnimalMission { Animal = a, MissionRoster = row };
                    row.Animals.Add(am);
                    context.AnimalMissions.Add(am);
                }
            }

            // Get rid of those that should be removed
            foreach (AnimalMission am in animalsToRemove)
            {
                context.AnimalMissions.Remove(am);
            }
        ...
        protected override MissionRoster AddNewRow(Guid id)
        {
            ...
            context.MissionRosters.Add(row);
            return row;
        }

        protected override void RemoveRosterRow(MissionRoster row)
        {
            context.MissionRosters.Remove(row);
        }
        ...
        public ActionResult TopResponders(string unit, string start, string end)
        {
            ...
            IQueryable<MissionRoster> source = context.MissionRosters;
            if (!string.IsNullOrEmpty(unit))
            {
                source = context.MissionRosters.Include(f => f.Unit);
            ...
            /*
var subResults = (from mr in context.MissionRosters.Include("Person").Include("Mission")
...
*/
        ...
        public ActionResult History()
        {
            ...
            using (var context = this.GetContext())
            {
                ...
                foreach (var row in (from m in context.Missions where m.StartTime > KcsarContext.MinEntryDate group m by new { Type = m.MissionType, Year = m.StartTime.Year } into g select new { Type = g.Key.Type, Year = g.Key.Year, Count = g.Count() }))
                {
                ...
                ViewData["volCount"] = (from m in context.MissionRosters
        ...
        public ActionResult Yearly(int? id)
        {
            ...
            foreach (var row in (from m in context.Missions
            ...
            var rosterByUnit = (from m in context.MissionRosters.Include(f => f.Mission).Include(f => f.Unit).Include(f => f.Person)
        ...
        public DataActionResult GetMissionRostersWithExpiredTraining(DateTime? start, DateTime? stop)
        {
            ...
            using (KcsarContext sandbox = new KcsarContext())
            {
                ...
                {
                    var rosters = (from mr in ctx.MissionRosters.Include(f => f.Mission).Include(f => f.Person) where mr.Mission.StartTime < stop.Value && mr.Mission.StartTime >= start.Value select mr);
        ...
        {
            return context.Missions.Include(f => f.Roster.Select(g => g.Unit)).Include(f => f.Roster.Select(g => g.Animals)).Include(f => f.Roster.Select(g => g.Person.Animals.Select(h => h.Animal)));
        }

        protected override void AddEventToContext(Mission newEvent)
        {
            context.Missions.Add(newEvent);
        }

        protected override void RemoveEvent(Mission oldEvent)
        {
            context.Missions.Remove(oldEvent);
        }
    }
}
