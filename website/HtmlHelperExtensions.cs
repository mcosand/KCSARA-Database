...
namespace System.Web.Mvc
{
  ...
  public static class HtmlHelperExtensions
  {
    ...
    public static MvcHtmlString EnumToNamedJson(this HtmlHelper helper, Type enumType)
    {
      ...
      foreach (object v in Enum.GetValues(enumType))
      {
        ...
        var displayAttr = enumType.GetMember(name)[0].GetCustomAttribute<DisplayAttribute>();
        string display = displayAttr == null ? name : displayAttr.Name;
        if (string.IsNullOrWhiteSpace(display)) continue;

        names.Add(name);
        sb.AppendFormat("'{0}':{{'d':'{1}','v':{2}}},", name, display, (int)v);
      }
      sb.AppendFormat("'_names':['{0}']}}", string.Join("','", names));
