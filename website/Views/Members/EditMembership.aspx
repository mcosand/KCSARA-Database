...
<asp:Content ID="aboutContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <% using (Html.BeginForm()) { %>
      ...
      <table>
        <tr><td>Unit:</td><td><%=
         ...
         <script type="text/javascript">$("#Unit").change(function () {GetStatusTypes();});</script></td></tr>
        <tr><td>Status:</td><td><div style="position:relative; display:inline;"><%= Html.DropDownList("Status").ToString() + Html.ValidationMessage("Status") + Html.Image("~/Content/images/progress.gif", new { id = "statusprogress", style = "visibility:hidden; position:absolute; left:0; width:100%; top:0; height:100%;" })%></div></td></tr>
        <tr><td>Activated:</td><td><%= Html.TextBox("Activated", ViewData.Model.Activated.ToShortDateString()).ToString() + Html.ValidationMessage("Activated")%>
            </td></tr>
        <tr><td colspan="2">Comments:<br /><%= Html.TextArea("Comments", new { style = "width:100%" }).ToString() + Html.ValidationMessage("Comments")%></td></tr>
        </table>
      <p>
      </p>
    ...
    <script type="text/javascript" src="<%= Url.Content("~/Content/script/jquery-1.2.6.js") %>" > </script>
