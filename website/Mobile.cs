...
namespace Kcsar.Web.Database
{
    ...
    public static class MobileHelpers
    {
        ...
        {
            return (c.HttpContext.Request.UserAgent != null && c.HttpContext.Request.UserAgent.IndexOf(agentToFind, StringComparison.OrdinalIgnoreCase) > 0);
        }
        ...
        public static void AddGenericMobile<T>(this ViewEngineCollection ves)
            where T : IViewEngine, new()
        {
            ves.Add(new CustomMobileViewEngine(c => c.IsMobileDevice() || c.UserAgentContains("iPhone") || c.UserAgentContains("iPod"), "Mobile", new T()));
        }
    }
}
